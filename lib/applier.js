...
export async function applyPlan({ plan, profile, remoteUrl, pushImpl, workDir = ".gogreen-work", planPath = "plan.json" }) {
  ...
  if (hasUpstream) {
    const isUnborn = await git.raw(["rev-parse", "--verify", "HEAD"]).then(() => false).catch(() => true);
    if (isUnborn) {
      await git.raw(["reset", "--hard", "origin/main"]);
    } else {
      const ahead = Number(await git.raw(["rev-list", "--count", "origin/main..HEAD"]).catch(() => "0"));
      if (ahead === 0) {
        const behind = Number(await git.raw(["rev-list", "--count", "HEAD..origin/main"]).catch(() => Infinity));
        if (behind > 0) {
          await git.raw(["reset", "--hard", "origin/main"]);
        }
      }
