...
function createRng(seed) {
  return seed !== undefined ? new Random(RNGFactory(String(seed))) : new Random();
}
...
export function generateDailyPlan({ now = new Date(), seed } = {}) {
  ...
  const minutes = [];
  for (let i = 0; i < count; i++) {
    let window = weightedWindow(rng);
    while (window.start * 60 > nowMinutes) {
      window = weightedWindow(rng);
    }
    const maxMinute = Math.min(
      (window.end - window.start) * 60,
      nowMinutes - window.start * 60,
    );
    minutes.push(window.start * 60 + rng.int(0, maxMinute));
  }
  minutes.sort((a, b) => a - b);
